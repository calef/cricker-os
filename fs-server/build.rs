...
fn main() {
    ...
    println!("cargo::rustc-link-arg-bin=fs_server=-T{dir}/../user/link.ld");
    ...
    println!("cargo::rustc-link-arg-bin=fs_server=-u_start");
    // Keep the ELF an ELF: the kernel's loader wants program headers, not a build-id note.
    println!("cargo::rustc-link-arg-bin=fs_server=--build-id=none");
}
