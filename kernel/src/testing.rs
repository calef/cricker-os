...
// signal is what lets `std_net` pass honestly: it spends its ~300 s in netd's *userspace* smoltcp
...
const SLOW_TESTS: &[(&str, u64)] = &[
    // Measured ~300 to 344 s on aarch64: a serial netd<->hellostd pipeline whose time is spent in
