...
//! - **Ranking is occurrence count and nothing else**, so a long page that mentions a word in
//!   passing can outrank a short page about it. Term frequency over document length would be one
//!   division, and it would need the page's length, which the layout does not store.
...
pub struct Found {
    ...
    origin: [u8; PATH_MAX],
    origin_len: u8,
    /// How many times the term occurs on this page. The whole of the ranking; see this module's
    /// `BUGS`.
    pub count: u16,
}
...
impl Ranked {
    ...
    pub fn offer(&mut self, bundle: &[u8], path: &[u8], title: &[u8], count: u16) {
        ...
        let mut at = 0;
        while at < self.n && self.found[at].count >= count {
            at += 1;
        }
        if at == RESULTS_MAX {
            return;
...
mod tests {
    ...
    fn a_search_keeps_the_best_sixteen_and_counts_the_rest() {
        ...
        // Sorted, and the strongest page is the one with the most occurrences.
