...
//!
//! This is the same move `gfx_proto::pixel` makes for rung one's test pattern and `compose::SCENE`
...
/// `user/src/vterm.rs` and the kernel wiring both assert the two agree at **compile time**, so a
