//! **uheap**: the userspace heap algorithm, as pure logic (milestone 27).
//!
...
//! compiles for the host, so these invariants are proven in milliseconds by `cargo test -p uheap`,
//! not in QEMU.
//!
