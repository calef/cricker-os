...
| `vnet` | **`virtio_net`** | the standard term for the device class, matching the `virtio` crate. The `v` in these two meant **different words**, `video`/`virtual` in one and `virtio` in the other, which is the same defect the `f`/`d`/`s` caretakers just shed. |
