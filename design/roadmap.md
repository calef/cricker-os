...
| 51 | PARTIAL | Wall-clock time, the `date` command, and an NTP service | lanes A, C and D built (two RTC drivers, the clock service with DECISIONS §43, the NTPv4 wire format, and the client that carries it): the machine knows what time it is, **reading the clock is a read-only page while setting it is a writable one**, and the network time client holds neither. `date` remains |
...
- **Still open:** the timed-wait fork below, and `date` itself (the calendar crate is built). NTP is
  no longer among them; see the NTP section.
