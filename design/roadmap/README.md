...
| 54 | BUILT | [A network file service a Mac can actually mount](54-network-file-service.md) | the first real workload with a real user, and the security claim backup servers deserve. A real Mac mounts it, reads and writes it, walks its subdirectories, sees its real free space, and **must prove who it is**: the adapter authenticates an NTLMv2 session against milestone 65's store while holding no key, which is what Samba's `smbd` cannot say. What remains is a *provisioning* path (the demo boot still admits guests and says so) and signing, both recorded in notes/smb.md's BUGS | 2026-08-17 |
...
| 62 | PARTIAL | [Tests that assert on time: make a red run mean something](62-time-sensitive-tests.md) | ~19 bounded spins (`for _ in 0..N { yield_now() }`) and wall-clock assertions flake under load. Four separate lanes and the integrator hit them on 2026-08-01; the CPU matrix multiplies the exposure fivefold | |
