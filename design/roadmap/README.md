...
| 100 | NOT-STARTED | [Read the machine's PSCI and its CPU list, not QEMU `virt`'s](100-psci-and-cpu-discovery.md) | The conduit (`hvc`), the function id, and the core list (`0..MAX_CPUS`, and `MAX_CPUS` is 4) are compiled in, where `/psci` and `/cpus` state all three and `crates/dtb` can already read them. The core-list half is a guaranteed silent no-op on a bigger board; the conduit half's failure is board-specific and untested. Milestones 24 and 88 both boot a machine that is not `virt` | |
| 101 | NOT-STARTED | [The L4 calibration, read from the IPC number that pays for the trap](101-el0-ipc-calibration.md) | Raised as "build the EL0 benchmark nobody owns"; `ipc_rtt_el0` has existed and been published for days (~2272 ns against the kernel-side ~951). What did not follow is the comparison: the L4 section still converts the kernel-side number, and redone against the one including the trap we are ~12 to 24 times an L4 fastpath, not 4 to 7. A measurement and a paragraph, and the headline gets worse | |
...
| 109 | NOT-STARTED | [`xargs`: batching a grant too large to hand over](109-xargs-at-the-grant-bound.md) | The glob bound is eight names, measured rather than chosen (sixteen ran the shell off its stack), and exceeding it is a loud refusal with nothing spawned. Milestone 47 will close without it and says why it is worth having: here `ARG_MAX` is a capability limit, not a buffer limit, so batching is the shape a bounded delegation takes rather than a workaround | |
| 110 | NOT-STARTED | [The recovery tool takes a device and a partition](110-recovery-from-a-partition.md) | `tools/redoxfs_host` reads a whole-device image; a real drive has a partition table at offset zero. `crates/gpt` and the recovery verbs both exist and nothing joins them, except in xtask's post-run check, which slices the partition into a temp file: twenty lines of the join, written in the wrong place. Milestone 57's residual, and small | |
