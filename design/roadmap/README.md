...
| 64 | PARTIAL | [Enough `std` to run somebody else's crate](64-std-for-real-crates.md) | Fifty crates.io crates measured: **39 build unchanged, 11 fail** (re-derived 2026-08-17; the old 35/15 double-counted four crates that fail in two classes at once), and the gaps are ranked by demand rather than by function count, which is the deliverable milestones 99 and 66 consume. **A green build is not evidence**: `tempfile` compiles, links, and returns "operation not supported", and `env::vars()` used to abort the process. Closed so far: five bindings, then `getrandom` (rank 1, `patches/getrandom-nife`, and `rand`/`uuid` now build), `env` (rank 4), `File::set_len` (8) and `fs::copy` (26). What remains is a **decision each**: the `std::os::unix` fallthrough (rank 2) wants a uid and an mtime set this system does not have, `thread::spawn` (3) is the block's own unanswered scheduling fork, `Metadata::modified` (19) is a wire-format change, and the rest wait on `File::open`'s resolution | |
