...
| 104 | NOT-STARTED | [The measurement continues past init](104-init-measures-what-init-loads.md) | The kernel measures the program it loads and nothing else, so the chain of trust stops at init's entry, and milestone 22 being BUILT makes that permanent rather than in-flight. The capability-correct fix is init measuring what init loads, in userspace, carrying its own table, which works because init's own bytes are measured. Named as the follow-up in three places, owned by nobody | |
