...
| 62 | PARTIAL | [Tests that assert on time: make a red run mean something](62-time-sensitive-tests.md) | ~19 bounded spins (`for _ in 0..N { yield_now() }`) and wall-clock assertions flake under load. Four separate lanes and the integrator hit them on 2026-08-01; the CPU matrix multiplies the exposure fivefold. The prescribed fixes shipped via milestone 78's rounds and milestone 50's shell work; the acceptance run ran 2026-08-17 (`script/repeat-under-load`, 45 full-suite runs at load average 26 to 63) and **did not pass**: 9 red, 8 of them the two timing assertions that need icount, twice each on both ISAs, and 1 a real double free of a frame in DESTROY's reclaim path. The icount instrument landed with 78 the same day, so what is left is not an instrument but a disposition: `script/icount` asserts zero missed ticks, which those two wall-clock assertions cannot, and deciding what they are still for is the remaining work | |
