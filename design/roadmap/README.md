...
| 98 | NOT-STARTED | [`sched` or `scheduler`: settle the abbreviation, once, everywhere](98-scheduler-naming.md) | The type is `Scheduler`, the module is `sched`, the static is `SCHED`: one concept, three lengths. Measured at **915 call sites and 109 references across 70 files**, the largest mechanical rename considered here. `sched` may belong with `elf` and `pci` as a term the field already knows; whichever way it goes, the point is to settle it so the question stops reopening | |
