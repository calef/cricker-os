...
[§42](#42-a-filesystem-declares-what-it-offers-and-must-be-truthful-it-is-not-required-to-be-capable)'s
...
propose a time but not set it, the clock's read/set/propose ladder ([§43](#43-reading-the-clock-is-a-page-setting-it-is-a-page-you-may-write-proposing-is-an-endpoint)), and now
...
gets `NO_ENTROPY` and the caller finds out, which is [§42](#42-a-filesystem-declares-what-it-offers-and-must-be-truthful-it-is-not-required-to-be-capable)'s no-silent-degradation rule applied to
the one payload where degrading quietly is worst.

### The bytes ride in the reply, not in a shared page

[§10](#10-process-model-capability-based-microkernel-untyped-memory-deferred) says bulk rides in a page and control rides in
