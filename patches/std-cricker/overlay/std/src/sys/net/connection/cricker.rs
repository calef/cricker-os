...
//! to the netd socket contract (DECISIONS §25, notes/net.md, `user/src/netproto.rs`).
//!
...
//! delegates it to netd (`SEND_CAP`, `OP_ATTACH_FRAME`); every later operation is one `CALL`
//! carrying a **socket id** and control words, with the payload already sitting in the shared
//! frame. netd drives smoltcp over the confined NIC and replies. Bytes never cross a message.
//!
//! **Blocking, one exchange at a time.** The contract is synchronous: a `CALL` blocks in netd
...
//! - **IPv6.** netd is IPv4-only (smoltcp built with `proto-ipv4`). A V6 address is `Unsupported`.
...
/// program image (0x40_0000), its stack (below 0x50_0000), and the heap (0x4000_0000). netd maps
...
// to netd (a once-per-id, sticky fact so open/close cycles reuse the frame without re-mapping an
// already-mapped VA), a per-TCP-socket residual buffer (bytes netd delivered into the frame that a
...
struct Slot {
    ...
    // the segment netd delivered). Served before the next `RECV`, so a stream never drops bytes.
    res_off: usize,
    res_len: usize,
    res: [u8; DATA_MAX],
...
struct Registry {
    ...
    // A round-robin cursor for id allocation. netd derives each socket's local port from its id
...
/// Ensure socket `id` has a shared frame delegated to netd. Once per id and sticky: the frame and
...
fn ensure_attached(id: u64) -> io::Result<()> {
    ...
    // Delegate it (narrowed to read/write) to netd with the ATTACH request. A negative result
...
/// granted, or `Other` if netd refused.
...
/// Close socket `id`: tell netd to drop it, then release the id (keeping its attached frame).
fn abandon(id: u64) {
...
impl TcpStream {
    ...
    pub fn socket_addr(&self) -> io::Result<SocketAddr> {
        // The local address (a DHCP-assigned IP, an ephemeral port netd picks) is not reported
...
impl UdpSocket {
    pub fn bind<A: ToSocketAddrs>(addr: A) -> io::Result<UdpSocket> {
        // The requested local address is validated (must be a numeric V4) but not honored: netd
