...
//! The serve loop it reaches implements one opcode. See user/src/credential.rs.
//!
...
/// A client's shared page. Must match user/src/credential.rs `VERIFY_VA`.
const PAGE_VA: u64 = 0x0000_0000_00e1_0000;
/// The provisioner's shared page. Must match user/src/credential.rs `PROV_VA`.
...
fn provisioner() -> ! {
    ...
    // A fourth identity in a three-slot store: FULL, not a silent overwrite of somebody.
    codes.push(request(
        PROV_VA,
        b"nobody",
        b"no room",
    ...
    done(codes, page_is_clean(PROV_VA))
}
...
fn honest() -> ! {
    ...
    done(codes, page_is_clean(PAGE_VA))
}
...
fn done(codes: Codes, clean: bool) -> ! {
    send(REPORT, RPT_DONE, codes.packed, u64::from(clean));
    exit()
}
