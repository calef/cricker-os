...
///
/// Two shapes (capsh::spawnproto). A **normal** job: the shell sends the request and, if `--mem`
...
fn spawn_service(
    ...
    loop {
        ...
        if interruptible {
            ...
            // is safe only because no manifest declares both today. `capsh` is where that stops
