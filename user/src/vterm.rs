...
//!
//! - [`MODE_DISPLAY`](vt::status::MODE_DISPLAY): it holds rung one's **display endpoint** and the
...
//! - [`MODE_WINDOW`](vt::status::MODE_WINDOW): it holds rung two's **doorbell** and one window's
...
/// would make. `vt::Vt::new` is `const` precisely so this can be a `static`.
static mut TERMINAL: vt::Vt = vt::Vt::new(1, 1);
...
fn paint(damage: vt::CellRect, stride: u32) -> (u32, u32, u32, u32) {
...
impl Wiring {
    ...
    fn present(&mut self, ring: bool) {
        ...
        if ring {
            let (r0, _) = call(PRESENT, compose::proto::req(compose::proto::COMMIT, 0), 0);
            if r0 as i64 != 0 {
                die(E_PRESENT);
            }
        }
...
pub extern "C" fn _start(mode: u64, _arg1: u64, _arg2: u64) -> ! {
    ...
    let (w, h, stride) = match mode {
        ...
        MODE_WINDOW => {
            ...
            let (r0, _) = call(PRESENT, compose::proto::req(compose::proto::HELLO, 0), 0);
            if r0 as i64 != 0 {
                die(E_HELLO);
            }
    ...
    if cols as usize > vt::MAX_COLS || rows as usize > vt::MAX_ROWS {
        die(E_GEOMETRY);
    }
    *term() = vt::Vt::new(cols, rows);
