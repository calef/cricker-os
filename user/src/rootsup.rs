...
//!
//! ```text
//!   rootsup   root untyped + the initrd + a report endpoint      (briefly)
//!     |
//!     +-- spawner   one program image, one budget, no initrd     (can build only that program)
//!     |
//!     +-- subsup    a request channel + a fault endpoint         (holds NO memory at all)
//!             |
...
//! ```
//!
//! After the handoff rootsup cannot build a process, cannot endow anyone with memory, and cannot
//! reach the sub-servers' internals. What is left of it is a supervisor so small it essentially
//! cannot fail: receive a death, report it, stop. A compromised rootsup at that point can lie about
//! deaths on one endpoint. That is the entire blast radius.
//!
//! The restart policy is `subsup`'s, in userspace, and the kernel never relaunches anything
...
pub extern "C" fn _start(_a0: u64, initrd_len: u64, _a2: u64) -> ! {
    ...
    let Some(subsup) = fs.read("subsup").and_then(|b| elf::Elf::parse(b).ok()) else {
        bail(3)
    };
