...
- **A correctly formatted `ratified` is never checked against Chris, and on 2026-08-14 two lanes
  contradicted each other about the same name on the same day.** `crates/cpu_set` was introduced
  twice in one stack of pull requests. #176 carried `Name: ratified (Chris, 2026-08-14, the same day
  ...
  sweep; Chris has not seen it)`. The second is the true one, and the two sat in the queue together
  asserting opposite things about whether a ruling had happened.
  ...
  **The gate caught the false one, and caught it for the wrong reason.** `script/names --check`
  ...
  cannot reach it. Written as `Name: ratified 2026-08-14 (Chris, …)`, the identical false claim would
  ...
  **This follows from the design and is not a defect in it.** The gate checks that a block names one
  ...
  Chris's ruling is as cheap as claiming anything else, and nothing downstream disagrees.** The
