...
download), then runs `cargo kani` over every crate carrying harnesses: **80 harnesses across 14
crates** as of milestone 51, in a few minutes. (The count had drifted: this line said 67 and the
README said 69 for the same 69 harnesses. Counted with `git grep -c kani::proof -- crates`.) Not in `script/bootstrap`, because the kernel build does
