...
| `script/lint` | Run clippy across the workspace with warnings denied (a CI gate), on both ISAs and in each boot-mode feature build, plus the non-clippy checks that share its job: broken intra-doc links, conflict markers, the roadmap status vocabulary, DECISIONS numbering, that every `script/` has an entry here, and that no file carries a module-wide `#![allow(dead_code)]` (DECISIONS §38). |
