...
| `script/names` | Who named this, when, and what was refused. Reads the `Name:` block every crate, program and `script/` entry point carries in its own header, so the table is computed rather than maintained. `--refused` lists every refused name and what holds the refusal, which is the query a proposer makes; `--unrecorded` lists the names nobody wrote a reason for; `<name>` answers for one name, from the refusals as well as from the tree; `--check` fails the build on a name with no block, and is gated in `lint`. **The name of this script is provisional.** See [naming.md](naming.md). |
